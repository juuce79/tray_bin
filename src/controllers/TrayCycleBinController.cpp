#include "TrayCycleBinController.h"
#include <windows.h>
#include <shlobj.h>
...
TrayCycleBinController::TrayCycleBinController(HWND hwnd) : m_model(), m_view(hwnd, &m_model) {
    m_view.Initialize();
    UpdateView();
...
void TrayCycleBinController::HandleCommand(WPARAM wParam) {
    switch (LOWORD(wParam)) {
        ...
        case ID_TRAY_EMPTY:
            if (!m_model.EmptyBin()) {
                MessageBox(NULL, L"Failed to Empty TrayCycle Bin", 
...
void TrayCycleBinController::HandleTrayMessage(LPARAM lParam) {
...
void TrayCycleBinController::UpdateView() {
